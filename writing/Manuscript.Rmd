...
Zoop_data <- read.csv("../data/Zoop_data.csv")
all_temperature <- read.csv("../data/all_temperature.csv")
```
...
if(file.exists("../data/HMSC_m.RData")){
...
Our experimental treatments resulted in a mean temperature difference of 2.95 \degree C ( $\pm 0.41$ \degree C SD) between the mesocosms without heaters and the mesocosms that received 300 W heaters (Figure \@ref(fig:temperature)). Ambient temperatures were moderate over the first five weeks of the experiment (until mid-July), with an average mean (max) daily temperature in the unheated mesocosms of `r all_temperature %>% filter(date < "2018-07-19", wattage == 0) %>% pull(temp_mean) %>% mean() %>% round(1)` \degree C (`r all_temperature %>% filter(date < "2018-07-19", wattage == 0) %>% pull(temp_max) %>% mean() %>% round(1)` \degree C; Figure \@ref(fig:temperature)). The warmest period of the experiment occurred from weeks six to eight (late July and early August), which resulted in an averaged mean (max) daily ambient temperatures of `r all_temperature %>% filter(date >= "2018-07-19", date <= "2018-08-02", wattage == 0) %>% pull(temp_mean) %>% mean() %>% round(1)` \degree C (`r all_temperature %>% filter(date >= "2018-07-19", date <= "2018-08-02", wattage == 0) %>% pull(temp_max) %>% mean() %>% round(1)` \degree C) and `r all_temperature %>% filter(date >= "2018-07-19", date <= "2018-08-02", wattage == 300) %>% pull(temp_mean) %>% mean() %>% round(1)` \degree C (`r all_temperature %>% filter(date >= "2018-07-19", date <= "2018-08-02", wattage == 300) %>% pull(temp_max) %>% mean() %>% round(1)` \degree C) in the 300W treatments. Ambient temperatures dropped following this warm period, continuing to decline over the final four weeks of the experiment, so that mean (max) daily temperatures in the unheated mesocosms averaged `r all_temperature %>% filter(date >= "2018-08-30", wattage == 0) %>% pull(temp_mean) %>% mean() %>% round(1)` \degree C (`r all_temperature %>% filter(date >= "2018-08-30", wattage == 0) %>% pull(temp_max) %>% mean() %>% round(1)` \degree C) in the final week.
...
All code and data for this study are available on https://github.com/plthompson/Zoop_metacom_exp_2018 and are archived on Dryad - doi to come.
...
predictions_abund %>% 
  filter(week == 8) %>% 
  ggplot(aes(x = temp, y = N/10, color = dispersal, fill = dispersal))+
  geom_line(size = 1)+
  ...
  scale_y_sqrt(breaks = c(1,round(seq(sqrt(1),sqrt(400), length = 13)^2, digits = -1)))+
  xlab("mean weekly temperature (°C)")+
  ylab(expression(paste("Abundance (Ind. ",L^{-1},")", sep = "")))
```
